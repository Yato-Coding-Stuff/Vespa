...
use crate::base::cli::presenter::events::InstallEvent;

#[derive(Debug, Error)]
pub enum PackageDownloaderError {
...
impl PackageDownloader {
    ...
    ) -> Result<TempDir, PackageDownloaderError> {
        let dir = TempDir::new()?;

        let parsed_url = url
            .parse::<reqwest::Url>()
            .map_err(|e| PackageDownloaderError::InvalidUrl(e.to_string()))?;

        let client = Client::new();
        let mut response = client.get(parsed_url).send()?;

        if !response.status().is_success() {
            return Err(PackageDownloaderError::Http(
